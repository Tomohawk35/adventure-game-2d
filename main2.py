...
# Game variables
level = 1
start_game = False # Set true to jump straight in
pause_game = False
start_intro = False
...
knight_button_img = scale_img(pygame.image.load("assets/images/buttons/button_knight.png").convert_alpha(), constants.BUTTON_SCALE)
ranger_button_img = scale_img(pygame.image.load("assets/images/buttons/button_ranger.png").convert_alpha(), constants.BUTTON_SCALE)
wizard_button_img = scale_img(pygame.image.load("assets/images/buttons/button_wizard.png").convert_alpha(), constants.BUTTON_SCALE)
...
# Create screen fades
intro_fade = ScreenFade(1, constants.BLACK, 6)
...
knight_button = Button(constants.SCREEN_WIDTH // 2 - 175, constants.SCREEN_HEIGHT // 2 - 200, knight_button_img)
ranger_button = Button(constants.SCREEN_WIDTH // 2 - 175, constants.SCREEN_HEIGHT // 2 - 0, ranger_button_img)
wizard_button = Button(constants.SCREEN_WIDTH // 2 - 175, constants.SCREEN_HEIGHT // 2 + 150, wizard_button_img)
...
while run:
    ...
    if start_game == False:
        ...
        if exit_button.draw(screen):
            run = False
    elif character_created == False:
        while character_class == None:
            if ranger_button.draw(screen):
                character_class = "Ranger"
            if knight_button.draw(screen):
                character_class = "Knight"
            if wizard_button.draw(screen):
                character_class = "Wizard"
        # Create player
        player = Character("Hero", character_class)
        # TODO: create character class selector
        ...
        if pause_game == True:
            ...
            if player.alive:
                ...
                arrow = bow.update(player, player_sprite)
                if arrow:
                ...
                for arrow in arrow_group:
                    ...
                    if damage: 
                        damage_text = DamageText(damage_pos.centerx, damage_pos.y, str(damage), constants.RED, game_font)
                        damage_text_group.add(damage_text)
                        hit_fx.play()
                damage_text_group.update(screen_scroll)
                fireball_group.update(screen_scroll, player)
                item_group.update(screen_scroll, player, player_sprite, coin_fx, heal_fx)
